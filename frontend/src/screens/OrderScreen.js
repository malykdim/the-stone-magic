...
const OrderScreen = ({ match, history }) => {
    ...
    }, [successPay, successDeliver, order, orderId, dispatch, history, userInfo]);
                                ...
                                <ListGroup variant='flush'>
                                    {order.orderItems.map((item, index) => (
                                        <ListGroup.Item key={index}>
                                            <Row>
                                                <Col md={2}>
                                                    <Image src={item.image} alt={item.caption} fluid rounded />
                                                </Col>
                                                <Col>
                                                    <Link to={`/mosaic/${item.mosaic}`}>
                                                        {item.caption}
                                                    </Link>
                                                </Col>
                                                <Col md={5}>
                                                    {item.qty} x {item.price} &euro; = {item.qty * item.price} &euro;
                                                </Col>
                                            </Row>
                                        </ListGroup.Item>
                                    ))}
